                ...
                <div class="card border rounded shadow w-full">
                    <div class="flex items-center justify-between px-6 py-4 border-b">
                        <h4 class="text-lg font-semibold">
                            Thêm câu hỏi mới cho
                            {{ $examSet->ExamSetName }}
                            ({{ $examSet->licenseType_ExamSet->pluck("LicenseTypeName")->join(", ") }})
                        </h4>
                         <button type="submit"
                                ...
                                @foreach ($questionCategory as $category)
                                    <div class="flex items-center mb-2 w-full gap-2">
                                        ...
                                        <div class="w-1/12 flex justify-center category-checkbox">
                                            <input type="checkbox" name="selected_categories[]" id="checkbox-{{ $category->CategoryID }}"
                                                value="{{ $category->CategoryID }}" class="form-checkbox text-blue-600 w-5 h-5">
                                        </div>

                                        <!-- Label -->
                                        <label for="checkbox-{{ $category->CategoryID }}" class="w-7/12 text-sm font-medium text-gray-800">
                                            {{ $category->CategoryName }}
                                        </label>

                                        <!-- Input số lượng -->
                                        <div class="w-4/12">
                                            <input type="number" name="amounts[{{ $category->CategoryID }}]" min="0" placeholder="Số lượng"
